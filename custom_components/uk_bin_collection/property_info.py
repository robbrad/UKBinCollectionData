...
async def async_get_property_info(lat, lng):
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(google_url, timeout=10) as google_resp:
                if google_resp.status != 200:
                    _LOGGER.warning(
                        f"Google Geocode API returned status {google_resp.status}"
                    )
                    return None
        ...
        if google_data.get("status") in [
            ...
            _LOGGER.error(
                f"Google Geocode API error: {google_data['status']} - {error_message}"
            )
            return None
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(postcodes_url, timeout=10) as postcodes_resp:
                if postcodes_resp.status != 200:
                    _LOGGER.warning(
                        f"postcodes.io API returned status {postcodes_resp.status}"
                    )
                    return None
        ...
        return {
            ...
            or "",  # Return empty string if postal_town not found
        }
