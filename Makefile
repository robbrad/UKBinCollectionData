...
integration-tests: ## runs unit tests for the project
	poetry run coverage run --omit "*/tests/*" -m behavex --parallel-processes 4 -D runner.continue_after_failed_step=true --format=allure_behave.formatter:AllureFormatter -o build/$(matrix)/allure-results uk_bin_collection/tests/features/
