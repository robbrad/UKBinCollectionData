import uk_bin_collection.get_bin_data as gbd
...
def mocked_requests_get(*args,**kwargs):
    class MockResponse:
        ...
        def raise_error(self,errorType):
            if errorType=='HTTPError':raise req_exp.HTTPError()
            elif errorType=='ConnectionError':raise req_exp.ConnectionError()
            elif errorType=='Timeout':raise req_exp.Timeout()
            elif errorType=='RequestException':raise req_exp.RequestException()
            return errorType
    if args[0]=='aurl':return MockResponse('test_data',200,None)
    elif args[0]=='HTTPError':return MockResponse({},999,'HTTPError')
    elif args[0]=='ConnectionError':return MockResponse({},999,'ConnectionError')
    elif args[0]=='Timeout':return MockResponse({},999,'Timeout')
    elif args[0]=='RequestException':return MockResponse({},999,'RequestException')
    elif args[0]=='notPage':return MockResponse('not json',200,None)
...
@pytest.mark.parametrize('url',['HTTPError','ConnectionError','Timeout','RequestException'])
@mock.patch('requests.get', side_effect=mocked_requests_get)
