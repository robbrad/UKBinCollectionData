...
def test_parse_data_preserves_multi_word_bin_names_other_than_food_waste():
    ...
    assert result["bins"][1]["type"] == "Garden Waste Bin"


def test_parse_data_skips_a_dateless_card_with_no_recognisable_frequency():
    html = RESULTS_HTML.replace("Thursday", "Someday")
    result = parse_fixture(html, real_datetime(2026, 7, 19, 10, 0))

    assert [b["type"] for b in result["bins"]] == [
        "Brown Bin",
        "Blue Bin",
        "Black Bin",
        "Purple Bin",
