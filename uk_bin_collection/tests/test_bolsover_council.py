...
class TestDetermineBinCollection:
    ...
    def test_known_september_2026_collections_round_b(self):
        ...
        assert _determine_bin_collection(date(2026, 9, 22), self.WEEK_BLACK_B, self.WEEK_BANDG_B, "No") == "Black Bin"
        assert _determine_bin_collection(date(2026, 9, 29), self.WEEK_BLACK_B, self.WEEK_BANDG_B, "No") == "Burgundy Bin & Green Bin"

    def test_round_a_opposite_to_round_b(self):
        """Round A and Round B must always get opposite bin types on the same date."""
        for d in [date(2026, 9, 15), date(2026, 9, 22), date(2026, 9, 29), date(2026, 10, 6)]:
            bin_b = _determine_bin_collection(d, self.WEEK_BLACK_B, self.WEEK_BANDG_B, "No")
            bin_a = _determine_bin_collection(d, self.WEEK_BLACK_A, self.WEEK_BANDG_A, "No")
            assert bin_b != bin_a, f"{d}: RoundB={bin_b} and RoundA={bin_a} should differ"
    ...
    def test_week53_to_week1_transition_2026_2027(self):
        ...
        bin_w53 = _determine_bin_collection(w53_monday, self.WEEK_BLACK_B, self.WEEK_BANDG_B, "No")
        bin_w01 = _determine_bin_collection(w01_monday, self.WEEK_BLACK_B, self.WEEK_BANDG_B, "No")
    ...
    def test_week53_thursday_dec31_2026(self):
        ...
        jan7 = date(2027, 1, 7)
        bin_dec = _determine_bin_collection(dec31, self.WEEK_BLACK_B, self.WEEK_BANDG_B, "No")
        bin_jan = _determine_bin_collection(jan7, self.WEEK_BLACK_B, self.WEEK_BANDG_B, "No")
    ...
    def test_week52_to_week1_transition_2027_2028(self):
        ...
        bin_w52 = _determine_bin_collection(w52_monday, self.WEEK_BLACK_B, self.WEEK_BANDG_B, "No")
        bin_w01 = _determine_bin_collection(w01_monday, self.WEEK_BLACK_B, self.WEEK_BANDG_B, "No")
    ...
    def test_parity_stable_across_weekdays(self):
        ...
        expected = _determine_bin_collection(target, self.WEEK_BLACK_B, self.WEEK_BANDG_B, "No")
        for _ in range(5):
            assert _determine_bin_collection(target, self.WEEK_BLACK_B, self.WEEK_BANDG_B, "No") == expected
    ...
    def test_green_suspension_returns_burgundy_only(self):
        ...
        result = _determine_bin_collection(burgundy_green_date, self.WEEK_BLACK_B, self.WEEK_BANDG_B, "Yes")
    ...
    def test_no_suspension_returns_both(self):
        ...
        burgundy_green_date = date(2026, 9, 29)
        result = _determine_bin_collection(burgundy_green_date, self.WEEK_BLACK_B, self.WEEK_BANDG_B, "No")
