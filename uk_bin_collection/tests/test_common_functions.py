...
def holiday_effect(country_code, subdiv=None):
    if subdiv == "ENG":
        ...
        return {
            datetime(2023, 11, 30): "St Andrew's Day"
        }
    return {}
...
def test_update_input_json(mock_save_data, mock_load_data):
    update_input_json("test_council", "TEST_URL", "path/to/input.json", postcode="TEST_POSTCODE", uprn="TEST_UPRN", web_driver="TEST_WEBDRIVER", skip_get_url=True)
...
def test_update_input_json_ioerror(mock_save_data, mock_load_data):
    ...
    with patch('builtins.print') as mock_print:
        ...
        mock_print.assert_called_once_with("Error updating the JSON file: Unable to access file")
...
def test_update_input_json_jsondecodeerror(mock_save_data, mock_load_data):
    ...
    with patch('builtins.print') as mock_print:
        update_input_json("test_council", "TEST_URL", "path/to/input.json")
        mock_print.assert_called_once_with("Failed to decode JSON, check the integrity of the input file.")
...
def test_load_data_existing_file():
    ...
    with patch('builtins.open', m):
        with patch('os.path.exists', return_value=True):
            ...
            assert data == {"key": "value"}, f"Data was {data} instead of {{'key': 'value'}}"
...
def test_load_data_invalid_json():
    ...
    with patch('builtins.open', mock_open(read_data=mock_file_data)), \
         patch('json.load', side_effect=json.JSONDecodeError("Expecting ',' delimiter", '', 0)):
...
def test_save_data_to_file():
    ...
    with patch('builtins.open', mock_file):
        ...
        assert written_data == expected_data, "Data written to file does not match expected JSON data"
