...
class CouncilClass(AbstractGetBinDataClass):
    ...
    def parse_data(self, page: str, **kwargs) -> dict:
        ...
        post_header_str = "Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp," \
                          "image/apng," \
        ...
        post_response = requests.post(post_url, headers=post_headers, data=form_data, verify=False)
        ...
        for i in range(1, 3):
            calendar = soup.select(f"#wasteCalendarContainer > div:nth-child(2) > div:nth-child({i}) > div")[0]
            month = datetime.strptime(calendar.find_next("h2").get_text(), "%B %Y").strftime("%m")
            year = datetime.strptime(calendar.find_next("h2").get_text(), "%B %Y").strftime("%Y")

            pink_days = [day.get_text().strip() for day in calendar.find_all("td", class_="pink")]
            black_days = [day.get_text().strip() for day in calendar.find_all("td", class_="normal")]
