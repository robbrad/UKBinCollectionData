...
class CouncilClass(AbstractGetBinDataClass):
    ...
    def parse_data(self, page: str, **kwargs) -> dict:
        ...
        submit_btn = wait.until(
            EC.presence_of_element_located((By.XPATH, f"//button[contains(@class, 'mx-name-actionButton3')]"))
        )
        ...
        dropdown_options = wait.until(
            EC.presence_of_element_located((By.XPATH, f'//option[contains(text(), "{user_paon}")]'))
        )
        parent_element = dropdown_options.find_element(By.XPATH, "..")  # Using ".." to move up to the parent element
        ...
        submit_btn = wait.until(
            EC.presence_of_element_located((By.XPATH, f"//button[contains(@class, 'mx-name-actionButton5')]"))
        )
        ...
        results = wait.until(
            EC.presence_of_element_located((By.XPATH, f'//span[contains(@class,"collection-sub")]'))
        )
        ...
        for bin_item in bins:
            ...
            for elem in day_of_week_elements:
                if elem.text.strip() != bin_type:  # Avoid taking the bin type as the date
