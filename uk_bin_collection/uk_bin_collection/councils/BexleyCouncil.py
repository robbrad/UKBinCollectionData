...
class CouncilClass(AbstractGetBinDataClass):
    ...
    def parse_data(self, page: str, **kwargs) -> dict:
        ...
        response = requests.get('https://services.athomeapp.net/ServiceData/GetUserRoundJson', headers=headers)

        # 200 is OK. Sometimes it times out and gives this, but I'm not parsing HTTP codes
        if response.status_code != 200:
            raise ValueError("Error parsing API. Please check your email is correct and registered on the @Home app.")
        ...
        for item in json_data:
            ...
            for sched in item["nextcollectiondates"]:
                bin_collection = datetime.strptime(sched["datestring"], "%d %m %Y %H:%M")
