...
class CouncilClass(AbstractGetBinDataClass):
    ...
    def extract_styles(self, style_str: str) -> dict:
        return dict((a.strip(), b.strip())
                    for a, b in (element.split(':')
                                 for element in style_str.split(';') if element))
    ...
    def parse_data(self, page: str, **kwargs) -> dict:
        ...
        inputElement_hn = driver.find_element(
            By.ID, 'p_lt_ctl04_pageplaceholder_p_lt_ctl02_WasteCollectionCalendars_NCCAddressLookup_txtHouse')
        ...
        driver.find_element(
            By.ID, 'p_lt_ctl04_pageplaceholder_p_lt_ctl02_WasteCollectionCalendars_NCCAddressLookup_butLookup').click()
        ...
        bins_collected = list(map(
            str.strip,
            soup.find('span', id='p_lt_ctl04_pageplaceholder_p_lt_ctl02_WasteCollectionCalendars_spanRouteSummary').string.replace(
                'Routes found: ', '').split(',')
        ))
        ...
        for bin in bins_collected:
            ...
            bin_colour = self.extract_styles(
                style_str)['background-color'].upper()
        ...
        for table in calander_tables:
            ...
            month_and_year = rows[0].find(
                'table', class_='calCtrlTitle').find('td').string
