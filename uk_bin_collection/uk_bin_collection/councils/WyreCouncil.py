...
class CouncilClass(AbstractGetBinDataClass):
    ...
    def parse_data(self, page: str, **kwargs) -> dict:
        ...
        for item in bins:
            collection_title = ' '.join(item.find("h3", {"class": "bin-collection-tasks__heading"}).text.split(" ")[2:4])
            collection_date = datetime.strptime(remove_ordinal_indicator_from_date_string(
                item.find("div", {"class": "bin-collection-tasks__content"}).text.strip().replace("\n", " ")),
                                                "%A %d %B")
